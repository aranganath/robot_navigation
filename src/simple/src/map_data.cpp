...
int main(int argc, char **argv) {
    ...
    // Scan subscribers
    ros::Subscriber range = nh.subscribe("/scan",1000,scan_callback);
    while(ros::ok()){
        ...
        for(std::vector<std::pair<geometry_msgs::Point, int>>::iterator g_it=g_map.begin();g_it!=g_map.end();++g_it){
            for(std::vector<std::pair<geometry_msgs::Point, int>>::iterator l_it=l_map.begin();l_it!=l_map.end();++l_it){           
                if(!check_map_descrepancy(l_it->first, g_it->first, l_it->second, g_it->second)){
                    ...
                    geometry_msgs::Point pt = Pose.pose.pose.position;                    
                    occupancy_grid_utils::Cell begin= occupancy_grid_utils::pointCell(l_occ.info, Pose.pose.pose.position);
                    occupancy_grid_utils::Cell dest= occupancy_grid_utils::pointCell(l_occ.info, l_it->first);
                    boost::optional<occupancy_grid_utils::AStarResult> gen_path=\
                    occupancy_grid_utils::shortestPathAStar(l_occ,begin,dest).get();
                    label:
                    if(gen_path.is_initialized()){
                        occupancy_grid_utils::AStarResult Path=gen_path.get();  
                        std::vector<geometry_msgs::Point> _path;
                        for(int i=0; i<Path.first.size();++i){
                            _path.push_back(occupancy_grid_utils::cellCenter(l_occ.info,Path.first[i]));
                        }
                    }
                    else{
                        goto label;
